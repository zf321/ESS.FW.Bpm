...
namespace ESS.FW.Bpm.Engine.Form.Impl.Engine
{
    /// <summary>
    ///     <para>
    ///         A simple <seealso cref="IFormEngine" /> implementaiton which renders
    ///         forms as HTML such that they can be used as embedded forms
    ///         inside camunda Tasklist.
    ///     </para>
    ///     
    /// </summary>
    public class HtmlFormEngine : IFormEngine
    {
        /* elements */
        ...
        protected internal const string CamBusinessKeyAttribute = "cam-business-key";
        ...
        protected internal virtual void AddCommonFormFieldAttributes(IFormField formField, HtmlElementWriter formControl)
        {
            ...
            formControl.Attribute(ClassAttribute, FormControlClass).Attribute(NameAttribute, formFieldId);

            if (!formField.BusinessKey)
                formControl.Attribute(CamVariableTypeAttribute, typeName)
                    .Attribute(CamVariableNameAttribute, formFieldId);
            else
                formControl.Attribute(CamBusinessKeyAttribute, null);
