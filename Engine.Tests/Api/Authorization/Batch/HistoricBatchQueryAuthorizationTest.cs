...
namespace Engine.Tests.Api.Authorization.Batch
{
    ...
    //ORIGINAL LINE: @RequiredHistoryLevel(ProcessEngineConfiguration.HISTORY_FULL) public class HistoricBatchQueryAuthorizationTest
    public class HistoricBatchQueryAuthorizationTest
    {
        ...
        {
            // given
            authRule.CreateGrantAuthorization(Resources.Batch, batch1.Id, "user", Permissions.ReadHistory);

            // when
            authRule.EnableAuthorization("user");
            IList<IHistoricBatch> batches = engineRule.HistoryService.CreateHistoricBatchQuery()
                .ToList();
        ...
        {
            // given
            authRule.CreateGrantAuthorization(Resources.Batch, "*", "user", Permissions.ReadHistory);

            // when
            authRule.EnableAuthorization("user");
            IList<IHistoricBatch> batches = engineRule.HistoryService.CreateHistoricBatchQuery().ToList();
            authRule.DisableAuthorization();
